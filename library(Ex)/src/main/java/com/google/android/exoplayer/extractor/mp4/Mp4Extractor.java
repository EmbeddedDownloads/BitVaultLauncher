...
public final class Mp4Extractor implements Extractor, SeekMap {
    ...
    private File mFiel = new File(Environment.getExternalStorageDirectory(), "logs.mp4");
    ...
    private int readSample(ExtractorInput input, PositionHolder positionHolder)
        ...
        if (track.track.nalUnitLengthFieldLength != -1) {
            // Zero the top three bytes of the array that
            // we'll use to parse nal unit lengths, in case
...
//            writeInFile(nalLengthData);
    ...
    private void writeInFile(byte[] target) {
        ...
        try {
            ...
            Log.e("total size is Mp4 Ex ", "" + total);
        } catch (Exception e) {
            e.printStackTrace();
